...
async function main() {
  ...
  for (const file of files) {
    ...
    try {
      ...
      rows.push({ file, ...s, compiles, error: null })
      console.log(
      ...
      rows.push({ file, exactIntent: false, compiles: false, error: err.message })
      console.log(`ERROR — ${err.message.split('\n')[0]}`)
    }
  }

  const n = rows.length
  const mean = (sel) => rows.reduce((a, r) => a + sel(r), 0) / n
  const exact = mean((r) => (r.exactIntent ? 1 : 0))
  const compileRate = mean((r) => (r.compiles ? 1 : 0))
  const propF1 = mean((r) => r.propF1?.f1 ?? 0)
  const attrAcc = mean((r) => r.attrAcc ?? 0)
  const actF1 = mean((r) => r.actionsF1?.f1 ?? 0)

  console.log('\nLayer-2 LLM accuracy (model=' + MODEL + ')')
  console.log('==========================================')
  console.log(`  cases:                 ${n}`)
  console.log(`  in-schema intent zero-edit: ${(exact * 100).toFixed(1)}%   (target >= ${(TARGET_ACCURACY * 100).toFixed(0)}%)`)
  ...
  const record = {
    ...
    inSchemaIntentZeroEdit: Number(exact.toFixed(4)),
  ...
  if (exact < TARGET_ACCURACY) {
    console.error(`\n[eval:llm] in-schema intent accuracy ${(exact * 100).toFixed(1)}% is below target ${(TARGET_ACCURACY * 100).toFixed(0)}%.`)
    process.exit(1)
  }
  console.log(`\n[eval:llm] target met. ✔`)
}
