<template>
  <div class="example-form-file-upload">
    ...
    <a-modal v-model:open="txtPreviewVisible" title="文本内容预览" width="600px" ::mask-closable="true">
...
import { UploadOutlined, PictureOutlined, FileOutlined, FolderOpenOutlined, EyeOutlined, DownloadOutlined, DeleteOutlined } from '@ant-design/icons-vue'
import { EsForm } from '@es-plus/adapter-antdv'
import type { BtnConfig } from '@es-plus/adapter-antdv'
...
const getFileIconComponent = (filename: string) => {
  const ext = getExtension(filename)
  if (imageExtensions.includes(ext)) return PictureOutlined
  if (ext === 'zip' || ext === 'rar' || ext === '7z') return FolderOpenOutlined
  return FileOutlined
}
...
const handlePreview = (file: any) => {
  ...
  if (officeExtensions.includes(ext)) {
    message.info(`"${file.name}" 为 Office 文档，浏览器无法直接预览，请使用下载功能`)
    return
  }
}
...
const formItems = [
  {
    ...
    triggerRender: (h: any) => {
      return h(
        ...
        [
          h(UploadOutlined, { style: { fontSize: '18px', marginRight: '6px', color: '#409eff' } }),
          ...
          ? h('div', { class: 'file-actions' }, [
              isPreviewable(file.name)
                ? h('span', {
                    ...
                    h('span', { class: 'btn-icon' }, [h(EyeOutlined)]),
                    '预览'
                  ])
                : null,
              isZipFile(file.name)
              ...
              h('span', {
                ...
                h('span', { class: 'btn-icon' }, [h(DownloadOutlined)]),
                '下载'
              ]),
              h('span', {
                ...
                h('span', { class: 'btn-icon' }, [h(DeleteOutlined)]),
                '删除'
              ])
            ])
          : null
...
const configBtn: BtnConfig[] = [
  {
    ...
    click: (model: Record<string, unknown>) => {
      ...
      message.success(`提交成功！已上传 ${files.length} 个文件: ${names}`)
