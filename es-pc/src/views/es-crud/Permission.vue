<template>
  <div class="example-crud-permission">
    <div style="margin-bottom: 12px; padding: 8px 12px; background: #f5f7fa; border-radius: 4px;">
      ...
      <a-radio-group v-model:value="currentRole" size="small" @change="handleRoleChange">
        <a-radio-button value="admin">管理员</a-radio-button>
        <a-radio-button value="editor">编辑员</a-radio-button>
        <a-radio-button value="viewer">访客</a-radio-button>
      </a-radio-group>
...
<script setup>
import { EsCrudPage, configureEsPlus } from '@es-plus/adapter-antdv'
...
const handleConfirm = (key, data) => message.success(`[${key}] ${data.name}`)
const handleBtn = (key) => message.info(`按钮: ${key}`)
</script>
