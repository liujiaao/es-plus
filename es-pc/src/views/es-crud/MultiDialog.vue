...
const schema = {
  ...
  toolbarBtns: [
    { name: '新增', type: 'primary', icon: 'PlusOutlined', dialogKey: 'add' },
...
const handleDialogConfirm = (dialogKey, data) => {
  message.success(`[${dialogKey}] 确认: ${JSON.stringify(data)}`)
}

const handleBtnClick = (key, payload) => {
  if (key === 'export') message.info('导出当前查询结果')
  if (key === 'delete') message.warning(`删除 ID: ${payload?.id}`)
}
</script>
