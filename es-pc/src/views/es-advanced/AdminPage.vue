...
import { EsForm, EsTable, useDialog } from '@es-plus/adapter-antdv'
...
const queryItems = [
  { prop: 'keyword', label: '关键词', formtype: 'Input', span: 6, attrs: { placeholder: '姓名/邮箱', allowClear: true } },
  {
...
const queryBtns = [
  ...
  {
    name: '新增', type: 'success', icon: 'PlusOutlined',
...
const columns = [
  ...
  { prop: 'role', label: '角色', width: 100 },
  {
    ...
    render: (_, { row }) => h(Tag, { type: statusMap[row.status], size: 'small' }, () => statusText[row.status])
  },
...
const handleDelete = (row) => {
  dialog({
    ...
    configBtn: [
      { name: '取消', click: (_, { close }) => close() },
      {
        name: '删除', type: 'danger', icon: 'DeleteOutlined',
        click: (_, { close }) => {
