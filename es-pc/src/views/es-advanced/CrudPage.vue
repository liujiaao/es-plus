...
const pageSchema: any = {
  ...
  tableBtns: [
    { name: '下载', icon: 'DownloadOutlined', code: 1, dialogKey: 'download' },
    { name: '新增', type: 'primary', icon: 'PlusOutlined', code: 1, dialogKey: 'add' },
...
function handleDialogConfirm(dialogKey: string, data: Record<string, any>) {
  if (dialogKey === 'add') {
    message.success(`新增订单成功: ${data.goods || data.customer}`)
...
function handleBtnClick(key: string, payload?: Record<string, any>) {
  if (key === 'delete') {
    message.success(`已删除订单: ${payload?.orderNo}`)
    crudRef.value?.refresh()
  }
  if (key === 'import') {
    message.info('正在打开 Excel 导入...')
  }
}
</script>
