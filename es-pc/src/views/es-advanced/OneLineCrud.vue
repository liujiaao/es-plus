<template>
  <div class="example-one-line-crud">
    ...
    >
      <template #default>
        <div class="toolbar">
          <a-button type="primary" icon="PlusOutlined" @click="openDialog()">新增</a-button>
        </div>
      </template>
    </es-table>
    <div class="hint">
...
import { message, Tag, Modal, Alert } from 'ant-design-vue'
...
const handleDelete = async (row) => {
  try {
    await Modal.confirm({ title: '提示', content: `确定删除 "${row.title}"？`, okType: 'warning' })
  ...
  dialog({
    ...
    configBtn: [
      ...
      { name: '提交', type: 'primary', click: (_, { close, getRefs }) => {
        message.success(`${isEdit ? '编辑' : '新增'}成功（模拟）`)
        close()
        tableRef.value?.httpRequestInstance()
      }}
    ]
