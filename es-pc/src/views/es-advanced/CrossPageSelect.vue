<template>
  <div class="example-cross-page-select">
    ...
    >
      <template #default>
        <div class="toolbar">
          <a-button type="primary" :disabled="!selectedCount" @click="handleBatchAction">
            批量操作 ({{ selectedCount }} 条跨页选中)
          </a-button>
          <a-button @click="tableRef?.clearAllSelection?.()">清空所有选择</a-button>
        </div>
      </template>
    </es-table>
    <div class="hint">
...
import { ref } from 'vue'
import { message, Tag, Alert, Modal } from 'ant-design-vue'
...
const handleBatchAction = async () => {
  ...
  try {
    await Modal.confirm({ title: '跨页批量操作', content: `已跨页选中 ${rows.length} 条数据（来自多个分页），确定执行批量操作？`, okType: 'info' })
    message.success(`已处理 ${rows.length} 条数据（模拟）`)
