<template>
  <div class="example-row-approval">
    ...
    >
      <template #default>
        <div class="toolbar">
          <a-button type="primary" :disabled="!selectionCount" @click="openBatchDialog('approve')">批量通过 ({{ selectionCount }})</a-button>
          <a-button danger :disabled="!selectionCount" @click="openBatchDialog('reject')">批量驳回 ({{ selectionCount }})</a-button>
        </div>
      </template>
    </es-table>
  </div>
...
const columns = [
  ...
  { prop: 'applicant', label: '申请人', width: 90 },
  {
    ...
    render: (_, { row }) => <Tag type={typeMap[row.type]} size="small">{row.reason}</Tag>
  },
...
const handleSingleApproval = async (row, status) => {
  ...
  try {
    await Modal.confirm({
      title: '审批确认',
      content: `确定${label} "${row.applicant}" 的申请吗？`,
      okType: status === 'approved' ? 'success' : 'warning'
    })
    row.status = status
  ...
  dialog({
    ...
    configBtn: [
      ...
      { name: `确定${label}`, type: action === 'approve' ? 'success' : 'danger', click: (_, { close }) => {
        ...
        message.success(`已批量${label} ${selection.length} 条申请`)
        close()
