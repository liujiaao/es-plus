...
const queryItems = [
  { prop: 'keyword', label: '关键词', formtype: 'Input', span: 6, attrs: { placeholder: '搜索商品名称', allowClear: true } },
  { prop: 'category', label: '分类', formtype: 'Select', span: 6, dataOptions: categoryOptions, attrs: { allowClear: true } },
  { prop: 'priceRange', label: '价格', formtype: 'Select', span: 6, dataOptions: priceOptions, attrs: { allowClear: true } }
]
...
const queryBtns = [
  ...
  {
    name: '新增', type: 'success', size: 'small', icon: 'PlusOutlined',
    click: () => openAddDialog()
  },
  {
    name: '批量删除', type: 'danger', icon: 'DeleteOutlined', size: 'small',
...
const columns = [
  ...
  },
  {
    ...
    render: (_, { row }) => {
      return <div>
        <a-button type="primary" size="small" text onClick={() => openEditDialog(row)}>编辑</a-button>
        <a-button type="danger" size="small" text onClick={() => handleDelete(row)} style="margin-left: 8px">删除</a-button>
      </div>
    }
  }
]
...
const handleDelete = async (row) => {
  try {
    await Modal.confirm({ title: '提示', content: `确定删除 "${row.title}" 吗？`, okType: 'warning' })
...
const handleBatchDelete = async () => {
  ...
  try {
    await Modal.confirm({ title: '提示', content: `确定删除选中的 ${selection.length} 条数据吗？`, okType: 'warning' })
