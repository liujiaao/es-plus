<template>
  <div class="example-advanced-step-form">
    <a-steps :active="currentStep" finish-status="success" simple>
      <a-step title="基本信息" />
      <a-step title="配置信息" />
      <a-step title="确认提交" />
    </a-steps>
    ...
    <div class="step-content">
      ...
      <div v-if="currentStep === 2" class="review-section">
        <a-descriptions :column="2" border>
          <a-descriptions-item label="项目名称">{{ formData.name }}</a-descriptions-item>
          <a-descriptions-item label="项目类型">{{ formData.type }}</a-descriptions-item>
          <a-descriptions-item label="负责人">{{ formData.manager }}</a-descriptions-item>
          <a-descriptions-item label="优先级">{{ formData.priority }}</a-descriptions-item>
          <a-descriptions-item label="开始日期">{{ formData.startDate }}</a-descriptions-item>
          <a-descriptions-item label="预计结束">{{ formData.endDate }}</a-descriptions-item>
          <a-descriptions-item label="项目描述" :span="2">{{ formData.desc || '-' }}</a-descriptions-item>
        </a-descriptions>
      </div>
    </div>
    ...
    <div class="step-actions">
      ...
      <a-button v-if="currentStep === 2" type="success" @click="submitForm">提交</a-button>
    </div>
  </div>
</template>
...
import { message, Steps, Step, Descriptions, DescriptionsItem } from 'ant-design-vue'
