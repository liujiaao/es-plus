<template>
  <div class="example-auto-fit-height">
    <div class="demo-controls">
      <a-radio-group v-model:value="heightMode" size="small">
        <a-radio-button label="fixed">固定高度 400px</a-radio-button>
        <a-radio-button label="fixed600">固定高度 600px</a-radio-button>
      </a-radio-group>
...
const queryItems = [
  { prop: 'keyword', label: '关键词', formtype: 'Input', span: 6, attrs: { placeholder: '搜索商品', allowClear: true } },
...
const columns = [
  ...
  { prop: 'title', label: '商品名称', minWidth: 200 },
  {
    ...
    render: (_, { row }) => {
      ...
      return <Tag type={map[row.category] || 'info'} size="small">{row.category}</Tag>
    }
  },
