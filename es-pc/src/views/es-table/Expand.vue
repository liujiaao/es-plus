<template>
  <div class="example-expand-table">
    <div class="mode-switch">
      <a-radio-group v-model:value="showMode" size="small">
        <a-radio-button label="expand">展开行</a-radio-button>
        <a-radio-button label="tree">树形数据</a-radio-button>
      </a-radio-group>
    </div>
    <es-table
      ...
      <template #expand="{ row }">
        <div class="expand-content">
          <a-descriptions :column="2" border>
            <a-descriptions-item label="详细地址">{{ row.detailAddress }}</a-descriptions-item>
            <a-descriptions-item label="入职日期">{{ row.joinDate }}</a-descriptions-item>
            <a-descriptions-item label="部门">{{ row.department }}</a-descriptions-item>
...
import { ref } from 'vue'
import { Descriptions, DescriptionsItem, RadioGroup, RadioButton } from 'ant-design-vue'
