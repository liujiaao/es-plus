<template>
  <div class="example-callback-pipeline">
    <div class="toolbar">
      <a-button type="primary" size="small" @click="tableRef?.httpRequestInstance()">刷新数据</a-button>
      <a-button size="small" @click="tableRef?.httpRequestInstance({ department: '技术部' })">筛选技术部</a-button>
    </div>
    <es-table
      ref="tableRef"
...
const columns = [
  ...
  { prop: 'department', label: '部门' },
  {
    ...
    label: '状态',
    width: 80,
    render: (_, { row }) => h(Tag, { type: row.active ? 'success' : 'info', size: 'small' }, () => row.active ? '在职' : '离职')
  },
