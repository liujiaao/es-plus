<template>
  <div class="example-table-height">
    <div class="height-controls">
      <span>容器高度：</span>
      <a-radio-group v-model:value="containerHeight" size="small">
        <a-radio-button value="300px">300px</a-radio-button>
        <a-radio-button value="500px">500px</a-radio-button>
...
const columns = [
  ...
  { prop: 'department', label: '部门' },
  {
    ...
    label: '状态',
    width: 100,
    render: (_, { row }) => h(Tag, { color: row.status === 'active' ? 'success' : 'default' }, { default: () => row.status === 'active' ? '在职' : '离职' })
  },
  { prop: 'email', label: '邮箱' }
]
