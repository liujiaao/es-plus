<template>
  <div>
    ...
    <el-alert type="info" :closable="false" style="margin-bottom:10px"
      description="treeConfig：rowField 行ID，parentField 父ID，通过扁平数据构造树形；transform:true 自动转换" />
    <es-table
      ref="tableRef"
      :columns="columns"
...
const columns = [
  ...
  { prop: 'headCount', label: '人数', width: 100, align: 'center' as const },
  { prop: 'budget', label: '预算(万)', width: 110, align: 'right' as const },
]
...
// 扁平树形数据，vxe treeConfig.transform 自动构造树
const treeData = ref([
...
function getGrid() {
  return (tableRef.value as any)?.vxeInstance?.()
}

onMounted(() => nextTick(() => {
  // vxe-grid 内部 <vxe-table> 初始化需要极短时间，延迟 50ms 确保 setAllTreeExpand 有效
