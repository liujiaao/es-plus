<template>
  <div>
    <el-alert
      ...
      description="点击列头可排序；薪资列使用 formatter 格式化；状态列使用 vxeColumn.formatter 自定义 vxe 原生格式化"
    />
    <es-table
      :columns="columns"
...
const statusMap: Record<string, string> = { active: '在职', leave: '离职', trial: '试用期' }
...
const columns = [
  ...
  },
  {
    ...
    vxeColumn: {
      formatter: ({ cellValue }: any) => statusMap[cellValue] ?? cellValue,
    },
  },
  {
    prop: 'joinDate',
