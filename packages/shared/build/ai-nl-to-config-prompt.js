export const NL_TO_CONFIG_FEWSHOT = [
    {
        nl: '用户管理页面：查询条件有用户名、手机号、状态（启用/禁用）；表格展示用户名、手机号、头像、注册时间、状态；支持新增、编辑、删除。',
        reasoning: '状态是有限枚举 → Select + 固定 dataOptions；头像是图片 → Upload，明细字段不作查询 (inQuery:false)；注册时间是时间 → DatePicker，系统生成、不在表单 (inForm:false)。类型由含义推断而非关键词命中。',
        config: {
            name: 'UserManage',
            apiUrl: '/api/users',
            fields: [
    ...
    },
    {
        ...
        reasoning: '工具栏按钮的左右定位只由 code 决定：code:1=左、code:2=右（不要用 position）。金额的两位小数显示是 schema 无法声明的展示逻辑 → 落成 formatter 扩展点串，而不是丢弃。',
        config: {
            name: 'OrderManage',
            apiUrl: '/api/orders',
            fields: [
    ...
    },
    {
        ...
        reasoning: '目标框架是 vue2 → target:"vue2"。优先级和状态都是有限枚举 → Select + dataOptions；截止日期 → DatePicker。多枚举各自独立配置 options。',
        config: {
            name: 'TaskManage',
            apiUrl: '/api/tasks',
            target: 'vue2',
    ...
    },
    {
        ...
        reasoning: '只读 → actions 仅 ["view"]，并关闭操作列 operationColumn:false。所有字段都是展示用；时间范围查询用 DatePicker。',
        config: {
            name: 'AuditLog',
    ...
    },
    {
        ...
        reasoning: '按钮级权限 → permissions 映射（action → 权限码），这是 RBAC 门控，schema 原生支持，不要落成注释。创建时间系统生成 (inForm:false)。',
        config: {
            name: 'DeptManage',
 ...
 */
export function buildNlToConfigSystemPrompt() {
    const rules = [
...
//# sourceMappingURL=ai-nl-to-config-prompt.js.map