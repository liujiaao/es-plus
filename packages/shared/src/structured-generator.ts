...
function buildSchemaWrapper(config: StructuredCrudConfig, hasDelete: boolean, _hasDialog: boolean, renderFields: FieldConfig[]): string {
  const ts = config.typescript
  const tOpts: TableOpts = (config.tableOptions || {}) as TableOpts
  const lines: string[] = []
...
function buildSchemaWrapperNew(config: StructuredCrudConfig, renderFields: FieldConfig[], warnings: string[]): string {
  const ts = config.typescript
  const tOpts: TableOpts = (config.tableOptions || {}) as TableOpts
  const lines: string[] = []
  const hasDelete = config.actions.includes('delete')
