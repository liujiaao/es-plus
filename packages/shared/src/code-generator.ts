 ...
 */
function adaptCodeToAntdv(code: string): string {
  let out = code;
  ...
  out = out.replace(
    /h\(ElTag,\s*\{\s*type:\s*(row\.\w+ === 1)\s*\?\s*'success'\s*:\s*'danger'\s*\}/g,
    "h(Tag, { color: $1 ? 'green' : 'red' }"
  );
 ...
 */
function rewriteConfirmToModal(code: string): string {
  const re =
    /([ \t]*)ElMessageBox\.confirm\(([^)]*)\)\s*\n\s*\.then\(async \(\) => \{([\s\S]*?)\n[ \t]*\}\)\s*\n[ \t]*\.catch\(\(\) => \{\}\)/g;
  return code.replace(re, (_m, indent: string, args: string, body: string) => {
