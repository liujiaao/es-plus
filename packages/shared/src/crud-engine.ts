...
function getDataOptions(fieldName: string): any[] | undefined {
  ...
  if (/类型|分类|级别|来源|可见范围/.test(fieldName)) return TYPE_OPTIONS
...
function splitFields(str: string): string[] {
  ...
  return cleaned
    ...
    .filter(s => {
      if (!s) return false
      if (s.length > 10) return false                  // too long → likely a clause, not a field
      if (s.length < 2 && !/[a-zA-Z]/.test(s)) return false  // single Chinese char → likely noise
      if (NON_FIELD_TOKENS.has(s)) return false
      return true
    })
}

function parseFields(rawInput: string): ParsedField[] {
...
function parseActions(input: string): string[] {
  if (/只(?:查看|读|浏览)/.test(input)) return ['view']
  ...
  if (!text.trim()) return ['add', 'edit', 'delete']
  ...
  if (/批准|通过|approve/i.test(text)) actions.push('approve')
  if (/拒绝|驳回|reject/i.test(text)) actions.push('reject')
  if (/上架|下架|批量上下架/i.test(text)) actions.push('toggleListing')
 ...
 */
function detectFeatures(input: string): {
  featureHints: string[]
  optionPatches: Record<string, any>
} {
  const hints: string[] = []
 ...
 */
function normalizeSpans(formItems: any[]): void {
...
export function generateCrudConfig(input: string): GeneratedConfig {
  ...
  const columns = fields
    ...
    .map(f => {
      ...
      if (/金额|价格|amount|price/i.test(f.name)) col.align = 'right'
      return col
    })
