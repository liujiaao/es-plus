...
exports.buildFirstClassGridOptions = buildFirstClassGridOptions;
/**
 * 将 TableOptions 一等公民字段映射为 vxe-grid 局部配置
 * 纯函数，零框架依赖，可被 vue3 / adapter-antdv / vue2 包共享
 */
function buildFirstClassGridOptions(opts) {
    const result = {};
    // 合计行
    if (opts.showFooter) {
    ...
    // 行列合并（直传给 vxe-grid）
    if (typeof opts.spanMethod === 'function')
        result.spanMethod = opts.spanMethod;
    return result;
}
