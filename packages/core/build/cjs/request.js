...
exports.DEFAULT_CONFIG_FORM_FIELD_OUT = void 0;
exports.checkQueryFields = checkQueryFields;
exports.configFormField = configFormField;
exports.formatConfigOut = formatConfigOut;
exports.queryTableListMethod = queryTableListMethod;
exports.httpRequestFormInstance = httpRequestFormInstance;
exports.getEveryFormQueryField = getEveryFormQueryField;
const shared_1 = require("./shared");
const compat_1 = require("./compat");
/**
 * 默认字段映射（与 1.3.5 保持一致）
 */
exports.DEFAULT_CONFIG_FORM_FIELD_OUT = {
    total: 'records',
    pageSize: 'pageSize',
    current: 'pageNo',
    listData: 'rows',
 ...
 */
function checkQueryFields(obj) {
    const checkListKey = ['total', 'pageSize', 'current', 'listData'];
    if (!(0, shared_1.isObject)(obj))
        return false;
    // 4 个键必须全部存在，且每个值都是非空字符串
    return checkListKey.every((key) => {
        const v = obj[key];
 ...
 */
function configFormField(options = {}, fieldFieldOutput) {
    if ((0, shared_1.isObject)(options.configFormOut) &&
    ...
    if (typeof fieldFieldOutput === 'function') {
        const configFields = fieldFieldOutput({ ...exports.DEFAULT_CONFIG_FORM_FIELD_OUT });
        if (checkQueryFields(configFields)) {
            return configFields;
        }
    }
    return { ...exports.DEFAULT_CONFIG_FORM_FIELD_OUT };
}
/**
 * 把后端响应按字段映射规则提取出来
 *
 ...
 */
function formatConfigOut(row, keyList, options = {}, fieldFieldOutput) {
    ...
    if (!(0, shared_1.isObject)(row))
        return configDataOption;
    for (const [key, value] of Object.entries(configFieldOut)) {
        if (!keyList.includes(key))
            continue;
        const rowValue = row[value];
        const usedValue = rowValue !== undefined && rowValue !== null
            ? rowValue
            : (0, shared_1.findValueByKey)(row, value);
        if (key === 'listData') {
            configDataOption[key] = Array.isArray(usedValue) ? usedValue : [];
        }
        else {
 ...
 */
function queryTableListMethod(params, options = {}, httpRequestGlobal) {
    if (!(0, shared_1.isObject)(options.apiParams))
        return;
    if (!Object.keys(options.apiParams).length)
        return;
    if (!options.apiParams.url)
        return;
    const initFormParams = { ...params, ...(options.apiParams.model || {}) };
 ...
 */
function httpRequestFormInstance(model, options, rows, httpRequestGlobal, fieldFieldOutput) {
    return new Promise((resolve, reject) => {
        queryTableListMethod({ pageIndex: 1, pageSize: 1000, ...(model || {}) }, {
            ...(options || {}),
            success: (res) => {
                const configRows = formatConfigOut(res, ['total', 'listData'], rows, fieldFieldOutput);
                // 与原 es-eui 一致：剥一层 res.data；若 res 本身是数组则直接传 res
                const innerData = Array.isArray(res)
                    ? res
                    : res?.data;
                resolve({ data: innerData, configRows });
            },
            fail: (err) => {
                reject(err);
            },
 ...
 */
async function getEveryFormQueryField(rowsList, httpRequestGlobal, fieldFieldOutput) {
    try {
        if (!Array.isArray(rowsList))
            return [];
        const apiUrlList = rowsList.filter((it) => it && it.apiParams && (0, shared_1.isObject)(it.apiParams) && it.apiParams.url);
        const apiResult = [];
        const wrappedPromises = apiUrlList.map((option) => {
            const promiseThen = httpRequestFormInstance({ ...(option.apiParams?.model || {}) }, {
                httpRequest: option.httpRequest,
                apiParams: option.apiParams,
                ...(option.apiParams?.options || {}),
            }, option, httpRequestGlobal, fieldFieldOutput);
            return (0, shared_1.wrapPromise)(promiseThen);
        });
        ...
        results.forEach((item, index) => {
            ...
            const crtnFn = (0, compat_1.getCallback)(listenToCallBack, 'responseTransform');
            ...
            // 没有 crtn 或 crtn 返回非数组，再走 callOptionListFormat / 兜底
            if (!newListOptions) {
                if (typeof option?.callOptionListFormat === 'function') {
                    const formatted = option.callOptionListFormat(preExtractedList);
                    newListOptions = Array.isArray(formatted) ? formatted : preExtractedList;
                }
                else {
                    newListOptions = preExtractedList;
                }
            }
            apiResult.push({
                ...
                listData: Array.isArray(newListOptions)
                    ? newListOptions
                    : configRows?.listData || option?.dataOptions || [],
            });
        });
        return apiResult;
    }
