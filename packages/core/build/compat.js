 ...
 */
export function resolveFormLayProps(layoutProps) {
    if (!layoutProps)
        return {};
 ...
 * 判断按钮是否在右侧
 */
export function isButtonRight(btn) {
    return getButtonPosition(btn) === 'right';
}
/**
 * 判断按钮是否在左侧
 */
export function isButtonLeft(btn) {
 ...
 */
export function filterBtnProps(btn, extraOmit) {
    const omit = new Set(BTN_ORCHESTRATION_KEYS);
    if (extraOmit)
 ...
 */
export function getCallback(cb, name) {
    if (!cb)
        return undefined;
    // 1. 直接按名称查找
