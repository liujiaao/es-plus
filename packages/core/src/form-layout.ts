 ...
 * @returns 行列布局结果
 */
export function getRowColsAlgorithm(formItemList: FormItemOption[]): FormLayoutResult {
  let pre = 0
  const groupArrayList: FormItemOption[][] = []
  const columnRows: RowLayoutInfo[] = []
  ...
  const columRowIndexs: number[][] = groupArrayList.map((it) => it.map(() => 0))
 ...
 */
export function shouldShowFoldButton(layoutResult: FormLayoutResult, minFoldRows: number): boolean {
 ...
 */
export function getBtnColSpan(
  layoutResult: FormLayoutResult,
  ...
  const totalSpan = lastColumn.reduce(
    (sum, idx) => sum + (formItemList[idx]?.span || 24),
    0
  )
 ...
 */
export function applyFoldFlags(
  formItemList: FormItemOption[],
  layoutResult: FormLayoutResult,
  folded: boolean,
  minFoldRows: number
): Array<FormItemOption & { isFold: boolean }> {
  if (!folded) {
    return formItemList.map((it) => ({ ...it, isFold: false }))
  }
  const { columnNodeIndex } = layoutResult
  const lastFoldIndex =
    columnNodeIndex[minFoldRows - 1] ??
