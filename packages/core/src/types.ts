...
// ============================================================================
// vxe 引擎专属类型（一等公民 API，不依赖 vxe-table 安装即可获得类型提示）
// ============================================================================
...
export interface VxeExportConfig {
  ...
  /** 导出合并单元格 */
  isMerge?: boolean
  columnFilterMethod?: (params: { column: Record<string, unknown> }) => boolean
  dataFilterMethod?: (params: { row: Record<string, unknown> }) => boolean
}

/** 工具栏配置（含导出/刷新/自定义列/打印按钮） */
export interface VxeToolbarConfig {
  refresh?: boolean | { queryMethod?: () => void | Promise<void> }
  export?: boolean | { isPrint?: boolean }
  print?: boolean
  /** 全屏切换按钮 */
  zoom?: boolean
  /** 自定义列显示/隐藏按钮 */
  custom?: boolean
  slots?: { buttons?: string; tools?: string }
}
...
export interface VxeTreeConfig {
  ...
  lazy?: boolean
  /** 懒加载方法 */
  loadMethod?: (params: { row: Record<string, unknown> }) => Promise<unknown[]>
}
...
export interface VxeExpandConfig {
  ...
  lazy?: boolean
  /** 懒加载方法 */
  loadMethod?: (params: { row: Record<string, unknown> }) => Promise<void>
  /** 是否允许展开的判断方法 */
  toggleMethod?: (params: { row: Record<string, unknown> }) => boolean
}
