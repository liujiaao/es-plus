...
import type { ApiParams, FormItemOption, ModelData, ListenToCallBack } from './types'
 ...
 * 默认字段映射（与 1.3.5 保持一致）
 */
export const DEFAULT_CONFIG_FORM_FIELD_OUT: ConfigFormFieldOut = {
  total: 'records',
  pageSize: 'pageSize',
  current: 'pageNo',
  listData: 'rows',
 ...
 */
export function checkQueryFields(obj: unknown): boolean {
...
export function configFormField(
  ...
  if (
    ...
    return options.configFormOut as unknown as ConfigFormFieldOut
  }

  if (typeof fieldFieldOutput === 'function') {
    const configFields = fieldFieldOutput({ ...DEFAULT_CONFIG_FORM_FIELD_OUT })
    if (checkQueryFields(configFields)) {
      return configFields
    }
  }

  return { ...DEFAULT_CONFIG_FORM_FIELD_OUT }
}
 ...
 */
export function formatConfigOut(
  ...
  if (!isObject(row)) return configDataOption
  ...
  for (const [key, value] of Object.entries(configFieldOut)) {
    ...
    const usedValue = rowValue !== undefined && rowValue !== null
      ? rowValue
      : findValueByKey(row as Record<string, unknown>, value)
    ...
    if (key === 'listData') {
      ...
      configDataOption[key] =
        typeof usedValue === 'number'
          ? usedValue
          : parseInt(usedValue as string, 10) || 0
    }
  }
 ...
 */
export function queryTableListMethod(
  params: Record<string, unknown>,
  options: RequestConfig = {},
  httpRequestGlobal?: (params: Record<string, unknown>) => Promise<unknown>
): void {
  if (!isObject(options.apiParams)) return
  if (!Object.keys(options.apiParams).length) return
  if (!options.apiParams.url) return

  const initFormParams = { ...params, ...((options.apiParams.model as Record<string, unknown>) || {}) }
  const requestOption: Record<string, unknown> = { ...((options.apiParams.options as Record<string, unknown>) || {}) }
  ...
  requestFn(requestPayload)
    .then((res) => {
      ...
      const normalized = res && (isObject(res) || Array.isArray(res)) ? res : {}
      options.success(normalized as Record<string, unknown>)
    })
    .catch((e) => {
 ...
 */
export function httpRequestFormInstance(
  model: Record<string, unknown>,
  options: RequestConfig,
  rows: FormItemOption,
  httpRequestGlobal?: (params: Record<string, unknown>) => Promise<unknown>,
  fieldFieldOutput?: (defaults: ConfigFormFieldOut) => ConfigFormFieldOut
): Promise<{ data: unknown; configRows: Record<string, unknown> }> {
      ...
      { pageIndex: 1, pageSize: 1000, ...(model || {}) },
      {
        ...
        success: (res) => {
          ...
          const innerData = Array.isArray(res)
            ? res
            : (res as Record<string, unknown> | undefined)?.data
 ...
 */
export async function getEveryFormQueryField(
  rowsList: FormItemOption[],
  httpRequestGlobal?: (params: Record<string, unknown>) => Promise<unknown>,
  fieldFieldOutput?: (defaults: ConfigFormFieldOut) => ConfigFormFieldOut
): Promise<FormFieldOptionResult[]> {
  try {
    if (!Array.isArray(rowsList)) return []
    const apiUrlList = rowsList.filter(
      (it) => it && it.apiParams && isObject(it.apiParams) && it.apiParams.url
    )
    const apiResult: FormFieldOptionResult[] = []

    const wrappedPromises = apiUrlList.map((option) => {
      const promiseThen = httpRequestFormInstance(
        { ...((option.apiParams?.model as ModelData) || {}) },
        {
          httpRequest: option.httpRequest,
          apiParams: option.apiParams,
          ...((option.apiParams?.options as Record<string, unknown>) || {}),
        },
        option,
        httpRequestGlobal,
        fieldFieldOutput
    ...
    results.forEach((item, index) => {
      ...
      if (typeof crtnFn === 'function') {
        ...
        const crtnResult = crtnFn(preExtractedList) as unknown
      ...
      apiResult.push({
        ...
        listData: Array.isArray(newListOptions)
          ? newListOptions
          : (configRows?.listData as unknown[]) || (option?.dataOptions as unknown[]) || [],
      })
    })
