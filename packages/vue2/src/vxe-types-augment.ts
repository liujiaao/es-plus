/**
 * vxe-table v3.x 逃生舱类型增强（@es-plus/vue2）
 *
 * 此文件为 vxe-grid 逃生舱字段（vxeConfig / vxeColumn / vxeOn）提供完整的
 * VxeGridProps / VxeColumnProps 原生类型推断，无运行时开销。
 *
 * 使用方式（二选一）：
 *   1. 在项目入口 import '@es-plus/vue2/vxe-types-augment'
 *   2. 在 tsconfig.json 的 include 数组中添加此文件路径
 *
 * 注意：此文件依赖 vxe-table v3.x 包提供类型，仅在项目已安装 vxe-table@3.x 时生效。
 *
 * ── 一等公民 API（无需此文件即有类型） ──────────────────────────────────────
