...
function parseActions(input) {
    const actions = [];
    if (/新增|添加|创建|add|create/.test(input))
        actions.push('add');
    if (/编辑|修改|更新|edit|update/.test(input))
        actions.push('edit');
    if (/删除|移除|remove|delete/.test(input))
        actions.push('delete');
    if (/查看|详情|detail|view/.test(input))
        actions.push('view');
    if (/导出|export/.test(input))
        actions.push('export');
    if (/导入|import/.test(input))
        actions.push('import');
