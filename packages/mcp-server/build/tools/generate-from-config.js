...
const FieldConfigSchema = z.object({
    ...
    formatter: z
        .string()
        .optional()
        .describe("Extension point: a JS arrow-function source string for read-only cell formatting, e.g. \"(row) => row.amount.toFixed(2)\". Use this to express display logic the schema can't otherwise capture."),
    render: z
        .string()
        .optional()
        .describe("Extension point: a render-function source string for a fully custom cell/form control. Emit this (rather than dropping the requirement) when business logic exceeds the declarative schema."),
 ...
 */
export function buildSemanticWarnings(config) {
    const warnings = [];
    ...
    const mutating = (config.actions ?? []).some((a) => a === "add" || a === "edit");
    if (mutating && fields.length > 0 && fields.every((f) => f.inForm === false)) {
        warnings.push("actions include add/edit but every field has inForm:false — the add/edit form will be empty. Set inForm:true on the fields users should fill.");
    }
    // dialogs.formItems referencing props absent from the top-level field set.
    const dialogs = config.dialogs ?? {};
    for (const [dialogId, dlg] of Object.entries(dialogs)) {
        ...
        if (isMutatingDialog && formItems.length === 0) {
            warnings.push(`dialog "${dialogId}" is an add/edit dialog but has no formItems — the modal body will be empty. Add formItems (usually the mutable subset of fields) so users have inputs to fill.`);
        }
        for (const fi of formItems) {
            if (fi?.prop && !topProps.has(fi.prop)) {
                warnings.push(`dialog "${dialogId}" form item "${fi.prop}" is not in the top-level fields — confirm this is intentional (it won't map to a table column).`);
            }
        }
    }
    ...
    for (const f of fields) {
        ...
        const hasOptions = (Array.isArray(f.dataOptions) && f.dataOptions.length > 0) || !!f.apiParams;
        if (needsOptions && !hasOptions) {
            warnings.push(`field "${f.prop}" is a ${f.formtype} but has no dataOptions or apiParams — it will render an empty dropdown. Provide dataOptions (fixed enum) or apiParams (remote source).`);
        }
    }
    return warnings;
}
export function registerGenerateFromConfig(server) {
    server.tool("generate_crud_from_config", "Generate a production-ready CRUD page from a STRUCTURED config (typed fields — not a JSON blob). This is the PRIMARY generation path: prefer it over generate_crud_page (which uses regex-based NL parsing and is a no-LLM fallback). You (the AI client) do the reasoning — read the natural-language request, then FILL these typed fields directly: name (PascalCase), apiUrl (real endpoint), fields[] (prop/label/formtype + inQuery/inTable/inForm), actions[]. The field descriptions tell you how to map meaning → formtype. For requirements the schema can't express, emit a marked extension point (formatter/render string) rather than dropping them. Supports target:'vue3'|'vue2'|'antdv' (default vue3). Read esplus://conventions, esplus://examples/nl-to-config and esplus://types first; after drafting, self-review against the original request before calling.", configShape, async (config) => {
        try {
