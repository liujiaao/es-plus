...
export function detect(pkgJsonText) {
    let pkg;
    try {
        ...
        return {
            target: "vue3",
            confidence: "low",
            reasoning: "Could not parse the provided package.json — falling back to vue3 default. " +
    ...
    if (deps["@es-plus/vue2"] && deps["@es-plus/vue3"]) {
        return {
            target: "vue3",
            confidence: "low",
            reasoning: "Both @es-plus/vue2 and @es-plus/vue3 declared — ambiguous. " +
    ...
    if (deps["es-plus-ui"]) {
        return {
            target: "vue3",
            confidence: "high",
            reasoning: "Project depends on es-plus-ui — the legacy name for what is now @es-plus/vue3. " +
    ...
    if (vueMajor === 3 || hasElementPlus) {
        return {
            ...
            reasoning: `Detected ${vueMajor === 3 ? `vue@${vueMajor}` : ""}${vueMajor === 3 && hasElementPlus ? " + " : ""}${hasElementPlus ? "element-plus" : ""} — ` +
    ...
    return {
        target: "vue3",
        confidence: "low",
        reasoning: "Could not find vue / element-plus / element-ui / @es-plus/* in dependencies — " +
...
//# sourceMappingURL=detect-project-target.js.map