...
const FieldConfigSchema = z.object({
  ...
  formtype: z.enum(VALID_FORM_TYPES as unknown as [string, ...string[]]),
  ...
  server.tool(
    ...
    "Generate a production-ready CRUD page from a structured JSON config. Unlike generate_crud_page (which uses regex-based NL parsing), this tool accepts precise field definitions, real API URLs, data options, and validation rules — producing zero-TODO code ready for production. The AI client should read esplus://conventions and esplus://types resources first, then construct the config JSON from the user's requirements.",
    {
      config: z
        .string()
        .describe(
          "JSON string of StructuredCrudConfig. Must include: name (PascalCase), apiUrl (real endpoint), fields (array with prop/label/formtype), actions (array of CRUD operations). See esplus://conventions resource for the full schema and examples."
        ),
    },
      ...
      try {
        ...
        const configSchema = z.object({
          ...
          actions: z
            .array(
              z.enum(
                VALID_CRUD_ACTIONS as unknown as [string, ...string[]]
              )
            )
            .min(1),
          tableOptions: z
        ...
        const result = generateFromConfig(validationResult.data as any);
