...
export function registerGenerateCrudPage(server: McpServer) {
  server.tool(
    "generate_crud_page",
    "Generate a complete Vue 3 CRUD page (.vue SFC) from a natural language description. Supports Chinese and English input. Produces query form, table columns, action buttons, and dialog forms.",
    {
      description: z
        .string()
        .describe(
