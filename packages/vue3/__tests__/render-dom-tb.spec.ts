...
describe('RenderDomTb (vue3)', () => {
  ...
  it('row=null → render 回退，vnode.children 含 "-"', () => {
    const vnode = render({ row: null, index: 0, dataKey: '', render: vi.fn() })
    // children 可能是字符串或数组，均包含 '-'
  ...
  it('dataKey 为空字符串 → value = null', () => {
    const renderFn = vi.fn().mockReturnValue(null)
    const row = { price: 42 }
    render({ row, index: 0, dataKey: '', render: renderFn })
    expect(renderFn).toHaveBeenCalledWith(
      expect.any(Function),
  ...
  it('row 为空对象（非 falsy）→ 调用 render，不走 null 分支', () => {
    const renderFn = vi.fn().mockReturnValue(null)
