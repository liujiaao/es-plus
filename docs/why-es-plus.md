...
> 一句话总结：**ES-Plus 把"表单—表格—弹窗"这条中后台最高频的链路抽象成一份 JSON Schema，把同一页面里 115 行模板与事件胶水代码压到 27 行配置（−76.5%），让 AI 写出的代码第一次就能在 CI 里通过 `vite build`，让同一份配置在 Vue 3 + Element Plus、Vue 2 + Element UI 和 Vue 3 + Ant Design Vue 三个渲染器之间零成本切换。**
...
以一个 **4 字段查询 + 4 列表格（含操作列）+ 分页** 的「用户管理」页面为例（极常见，几乎所有中后台都有）：

| 模块 | 原生 Element Plus 写法 | 实测行数 |
|------|----------------------|------|
| 模板区 | `<el-form>` + 4 个 `<el-form-item>` + 控件 + 查询/重置按钮；`<el-table>` + 4 个 `<el-table-column>`（含状态 Tag 的 scope slot）+ 操作列 + `<el-pagination>` | **61 行** |
| 事件处理与取数 | `fetchDeptOptions` / `fetchData` / `handleQuery` / `handleReset` / `handlePageChange` / `handleSizeChange` / `handleEdit` / `handleDelete` | **54 行** |
| 其余（imports / state / 常量） | — | **36 行** |
| **合计** | | **151 行** |

**ES-Plus 侧写同样的页面是 85 行**（模板 18 + 事件 9 + 其余 58）：其中查询 / 重置 / 翻页 / 取数**一行事件代码都不用写**，`triggerEvent: true` 全自动接管。

更关键的是：那 **115 行模板与事件胶水代码**（61 + 54）正是每个 CRUD 页面里长得一模一样、只换字段名的部分 —— 换成配置后只剩 **27 行**（−76.5%）。任何一个有 20 个 CRUD 页面的项目，光这一项就有 **1760 行可以蒸发的代码**。

> 以上行数由 `npm run one-config:gen` 从 `docs/brand/one-config/{native,esplus}.vue` **实测**得出，
> 由 `npm run check:site-claims` 守住；两个样例是同一页面、同一功能范围，无省略号、无 stub。
> 口径：模板区行数 + `handle*`/`fetch*` 事件与取数函数行数。整页非空行口径为 136 → 76（−44.1%）。

### 1.2 这些"死代码"的隐性成本

数得清的：

- **写**：1760 行 × 1 分钟/行 = 29 小时
- **审**：1760 行 × 0.5 分钟/行 = 15 小时
...
| 单页代码量 | 151 行 | 85 行（其中模板与事件胶水 115 → 27） |
...
|---|------------|---------|-----|
| 30 个页面的代码量 | 30 × 151 = 4530 行 | 30 × 85 = 2550 行 | **1980 行** |
| 30 个页面的写代码工时 | 4530 ÷ 30 行/小时 = 151 小时 | 2550 ÷ 30 = 85 小时 | **66 小时** |
| Code Review 工时 | 4530 ÷ 100 行/小时 = 45 小时 | 2550 ÷ 100 = 26 小时 | **19 小时** |

**直接节省：~85 小时 ≈ 2 个工作周**（按整页行数外推；若只看模板与事件胶水那一项，节省比例更高）
